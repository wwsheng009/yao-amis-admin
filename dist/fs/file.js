...
function buildTree(folders) {
    const root = { label: "", children: [], value: "" };
    for (const folder of folders) {
        let currentLevel = root;
        const path = folder.split("/").filter(Boolean);
        for (const folderName of path) {
            let existingFolder = currentLevel.children.find((child) => child.label === folderName);
            if (!existingFolder) {
...
//   return false;
// }
function filterUserAuthFolderList(type, folderList) {
    if (type === "user" || type === "public") {
        return folderList;
    }
    // 超级用户没有限制
    ...
    const folders = foldersAuth.map((f) => `${uploadDir}${f}`);
    ...
    folderList = folderList.filter((f) => folders.some((f1) => {
        // 权限目录长于需要检查的目录
 ...
 */
function targetOperationAuthCheck(type, targetIn, operation) {
    if (type === "user" || type === "public") {
        return;
    }
    // 超级用户没有限制
    ...
    const foldersAuth = authObjects.method_with_folder["ANY"].concat(authObjects.method_with_folder[operation] || []);
    const folders = foldersAuth.map((f) => `${uploadDir}${f}`);
    // const folder_method = authObjects.folder_method;
    // // console.log("folder_method", folder_method);
    // if (
    ...
    if (!found) {
        throw new Exception(`目录:${target.substring(uploadDir.length)} 操作:${operation} 未授权`);
    }
}
/**
 * 获取需要授权的目录的结构列表
...
function getPermissionFolderTree() {
    ...
    let list = Process("fs.system.ReadDir", rootFolder);
 ...
 * @returns
 */
function UploadFile(type, file, folder) {
...
function batchDeleteFile(type, payload) {
...
function getBasename(filename, noEscape) {
    if (filename == null) {
        return "";
    }
...
function saveFile(type, file, folder) {
 ...
 * @returns
 */
function getFolderList(type, parent) {
    //没有读取授权
    ...
    const uploadFolder = parentDir != "" ? `${userDir}/${parentDir}/` : `${userDir}/`;
    if (!Process("fs.system.Exists", uploadFolder)) {
        return [];
    }
    let list = Process("fs.system.ReadDir", uploadFolder);
    list = list.map((l) => l.replace(/[\\/]+/g, "/"));
    let list2 = list.filter((dir) => Process("fs.system.isDir", dir));
 ...
 * @returns
 */
function fileSearch(type, parentFolder, querysIn, payload) {
    //没有读取授权
    // if (!checkUserCanReadAuth(type)) {
    //   return { items: [], total: 0 };
    // }
    ...
    list.forEach((f) => {
        ...
        if (isFile) {
            ...
            list2.push({
                ...
                name: baseName,
                path: fpath,
                url: `/api/v1/fs/${type}/file/download?name=${encodeURIComponent(fpath)}`,
                mime: mimeType,
...
function getFileList(type, folder, keywords) {
    if (folder == null) {
        folder == "";
    }
    folder = normalizeFolder(folder);
    ...
    if (keywords != null && keywords != "") {
        return list2.filter((f) => f.name.toLowerCase().includes(keywords.toLowerCase()));
    }
    return list2;
}
...
function createFolder(type, parent, folder) {
...
function deleteFolder(type, folder) {
    if (folder == null || folder == "") {
...
function moveFolder(type, source, target) {
